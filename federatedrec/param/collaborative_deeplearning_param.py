...
class CDLInitParam(InitParam):
    def __init__(self, embed_dim=10, init_method='random_normal'):
        super(CDLInitParam, self).__init__()
    ...
    def check(self):
        if type(self.embed_dim).__name__ not in ["int"] or self.embed_dim < 0:
            raise ValueError(
                "CDLInitParam's embed_dim {} not supported, should be 'int'"
                "and greater than 0".format(
                    self.embed_dim))
        return True


class CDLParam(BaseParam):
    """
    ...
    def check(self):
        descr = "collaborative_deeplearning's"
        ...
        if 'learning_rate' in self.optimizer.__dict__["kwargs"] and \
            ...
            raise ValueError(
                "collaborative_deeplearning's optimizer['learning_rate'] {} not supported, should be float type and greater "
        ...
        if 'decay' in self.optimizer.__dict__["kwargs"] and \
            ...
            raise ValueError(
                "collaborative_deeplearning's optimizer['decay'] {} not supported, should be float type and greater "
        ...
        if type(self.max_iter).__name__ != "int" or (isinstance(self.max_iter, int) and self.max_iter < 1):
            raise ValueError(
                "collaborative_deeplearning's max_iter {} not supported, should be int type and greater then 0".format(self.max_iter))
        elif self.max_iter <= 0:
            raise ValueError(
                "collaborative_deeplearning's max_iter must be greater than or equal to 1")
...
class HeteroCDLParam(CDLParam):

    def __init__(self,
                 optimizer={"optimizer": "SGD", "learning_rate": 0.01},
                 batch_size=-1, init_param=CDLInitParam(), max_iter=100,
                 early_stop={"early_stop": "diff"},
                 predict_param=PredictParam(), cv_param=CrossValidationParam(),
                 ):
        super(HeteroCDLParam, self).__init__(optimizer=optimizer,
    ...
    def generate_pb(self):
        pb = cdl_model_meta_pb2.HeteroCDLParam()
    ...
    def restore_from_pb(self, pb):
        ...
        self.loss = pb.loss
        self.init_param = CDLInitParam(embed_dim=pb.embed_dim)
        return pb
