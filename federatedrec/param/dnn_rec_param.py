...
class DNNRecInitParam(InitParam):
    def __init__(self, embed_dim=10, init_method='random_normal'):
        super(DNNRecInitParam, self).__init__()
    ...
    def check(self):
        if type(self.embed_dim).__name__ not in ["int"] or self.embed_dim < 0:
            raise ValueError(
                "DNNRecInitParam's embed_dim {} not supported, should be 'int'"
                "and greater than 0".format(
                    self.embed_dim))
        return True


class DNNRecParam(BaseParam):
    """
    ...
    def __init__(self,
        ...
        super(DNNRecParam, self).__init__()
    ...
    def check(self):
        ...
        if 'learning_rate' in self.optimizer.__dict__["kwargs"] and \
            ...
            raise ValueError(
                "dnn_rec's optimizer['learning_rate'] {} not supported, should be float type and greater "
        ...
        if 'decay' in self.optimizer.__dict__["kwargs"] and \
            ...
            raise ValueError(
                "dnn_rec's optimizer['decay'] {} not supported, should be float type and greater "
        ...
        if type(self.max_iter).__name__ != "int":
            raise ValueError(
                "dnn_rec's max_iter {} not supported, should be int type".format(self.max_iter))
        elif self.max_iter <= 0:
            raise ValueError(
                "dnn_rec's max_iter must be greater or equal to 1")
...
class HeteroDNNRecParam(DNNRecParam):
    """
    Parameters
    ----------
    ...
    def __init__(self, optimizer={"optimizer": "SGD", "learning_rate": 0.01},
                 ...
                 batch_size=-1, init_param=DNNRecInitParam(),
                 max_iter=100,
    ...
    def check(self):
        ...
        if not isinstance(self.aggregate_iters, int):
            raise ValueError(
                "dnn_rec's aggregate_iters {} not supported, should be int type".format(
                    self.aggregate_iters))

        return True

    def generate_pb(self):
        pb = dnnrec_model_meta_pb2.HeteroDNNRecParam()
    ...
    def restore_from_pb(self, pb):
        ...
        self.loss = pb.loss
        self.init_param = DNNRecInitParam(embed_dim=pb.embed_dim)
        return pb
