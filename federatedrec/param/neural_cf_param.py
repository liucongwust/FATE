...
from federatedrec.protobuf.generated import ncf_model_meta_pb2
...
class NCFParam(BaseParam):
    ...
    def check(self):
        ...
        if self.penalty is None:
            ...
            raise ValueError(
                "neural_cf's penalty {} not supported, should be str type".format(self.penalty))
        else:
            self.penalty = self.penalty.upper()
            if self.penalty not in [consts.L1_PENALTY, consts.L2_PENALTY, 'NONE']:
                raise ValueError(
                    "neural_cf's penalty not supported, penalty should be 'L1', 'L2' or 'none'")

        if not isinstance(self.tol, (int, float)):
            raise ValueError(
                "neural_cf's tol {} not supported, should be float type".format(self.tol))

        if not isinstance(self.neg_count, int):
            raise ValueError(
                "neural_cf's neg_count {} not supported, should be float type".format(self.neg_count))

        if type(self.alpha).__name__ not in ["float", 'int']:
            raise ValueError(
                "neural_cf's alpha {} not supported, should be float or int type".format(self.alpha))

        # if type(self.optimizer).__name__ != "str":
        #     raise ValueError(
        #         "neural_cf's optimizer {} not supported, should be str type".format(self.optimizer))
        # else:
        #     self.optimizer = self.optimizer.lower()
        #     if self.optimizer not in ['sgd', 'rmsprop', 'adam', 'adagrad', 'nesterov_momentum_sgd']:
        #         raise ValueError(
        #             "neural_cf's optimizer not supported, optimizer should be"
        ...
        if type(self.learning_rate).__name__ != "float":
            raise ValueError(
                "neural_cf's learning_rate {} not supported, should be float type".format(
                    self.learning_rate))
        ...
        if type(self.decay).__name__ not in ["int", 'float']:
            raise ValueError(
                "neural_cf's decay {} not supported, should be 'int' or 'float'".format(
                    self.decay))

        if type(self.decay_sqrt).__name__ not in ['bool']:
            raise ValueError(
                "neural_cf's decay_sqrt {} not supported, should be 'bool'".format(
                    self.decay_sqrt))
...
class HeteroNCFParam(NCFParam):
    ...
    def __init__(self, penalty='L2',
                 ...
                 batch_size=-1, learning_rate=0.001, init_param=NCFInitParam(),
                 max_iter=100, early_stop='diff',
                 predict_param=PredictParam(), cv_param=CrossValidationParam(),
                 decay=1, decay_sqrt=True,
                 aggregate_iters=1, validation_freqs=None
                 ):
        super(HeteroNCFParam, self).__init__(penalty=penalty, tol=tol, alpha=alpha, optimizer=optimizer,
